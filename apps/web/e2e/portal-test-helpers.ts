...
export const testAdminEmail =
  process.env.E2E_ADMIN_EMAIL ?? "e2e-owner@example.invalid";
export const testAdminPassword =
  process.env.E2E_ADMIN_PASSWORD ?? "E2E-Owner-Only-2026!";
...
export async function loginAsTestAdmin(page: Page) {
  ...
  await expect(
    page.getByRole("heading", {
      name: /project, meeting and private investor flow/i,
    }),
...
export async function csrfToken(request: APIRequestContext) {
  ...
  const cookie = state.cookies.find(
    (item) =>
      item.name === "ued_admin_csrf" || item.name === "__Host-ued-admin-csrf",
  );
  if (!cookie)
    throw new Error("Admin CSRF cookie is missing from the test context");
  return cookie.value;
}
...
export async function adminMutation(
  ...
  const headers = {
    ...
    origin: process.env.E2E_ORIGIN ?? "http://127.0.0.1:8090",
  };
...
export function invitationToken(shareUrl: string) {
  ...
  if (!token || token.length < 32)
    throw new Error("One-time invitation token was not present");
  return token;
}
