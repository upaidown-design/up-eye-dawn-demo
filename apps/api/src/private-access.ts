...
export async function registerPrivateAccess(app: FastifyInstance, {root}: {root: string}) {
  ...
  if (localPortalTestMode && !externalEnabled) nda.status = env('NDA_LEGAL_STATUS', nda.status) as NdaFile['status'];
  ...
  for (const document of ndaFiles) {
    ...
    await pool.query(`INSERT INTO private_portal.nda_documents(id,version,title,legal_status,jurisdiction,governing_law,signature_profile,content,content_sha256,reaccept_required,metadata) VALUES($1,$2,$3,$4,$5,$6,$7,$8,$9,true,$10)
      ON CONFLICT(version) DO NOTHING`, [randomUUID(), document.version, document.title, document.status, document.jurisdiction ?? 'UNSPECIFIED', document.governingLaw ?? 'TO_BE_SELECTED_BY_COUNSEL', document.signatureProfile ?? 'SIMPLE_ELECTRONIC_SIGNATURE_WORKFLOW', JSON.stringify(document), contentHash, JSON.stringify({source: `data/admin/${document.version === nda.version ? 'nda-v1.json' : document.jurisdiction === 'EU_EEA' ? 'nda-eu-v1.json' : 'nda-us-v1.json'}`})]);
  }
  if (externalEnabled && !(await pool.query("SELECT 1 FROM private_portal.nda_documents WHERE legal_status='APPROVED' LIMIT 1")).rowCount) throw new Error('External portal safety gate failed: at least one database NDA must be APPROVED');
  const startupNdaStatus = (await pool.query("SELECT CASE WHEN count(*) FILTER (WHERE legal_status='APPROVED')>0 THEN 'APPROVED' ELSE 'DRAFT_FOR_WORKFLOW_TESTING' END AS status FROM private_portal.nda_documents")).rows[0].status as NdaFile['status'];
  ...
  registerNdaAuthoringRoutes(app, {pool, requireAdmin, requireAdminMutation, audit});
  registerMailCenterRoutes(app, {pool, requireAdmin, requireAdminMutation, audit});
  ...
  app.get('/api/v1/admin/security', async (request, reply) => { if (!await requireAdmin(request, reply)) return; const [activeSessions, securityEvents, mfaStats, ndaStats] = await Promise.all([pool.query("SELECT count(*)::int AS count FROM private_portal.visitor_sessions WHERE status='ACTIVE' AND expires_at>now() AND idle_expires_at>now()"), pool.query("SELECT count(*)::int AS count FROM private_portal.audit_events WHERE severity='SECURITY' AND timestamp_utc>now()-interval '7 days'"), pool.query("SELECT count(*) FILTER (WHERE mfa_enabled=true AND mfa_secret_encrypted IS NOT NULL)::int AS enrolled,count(*)::int AS members FROM private_portal.admin_users WHERE status='ACTIVE'"), pool.query("SELECT count(*) FILTER (WHERE legal_status='APPROVED')::int AS approved,count(*)::int AS total FROM private_portal.nda_documents")]); const mfa = mfaStats.rows[0]; const ndaState = ndaStats.rows[0]; return {externalPortal: externalEnabled ? 'ENABLED' : workflowTestEnabled ? 'WORKFLOW_TESTING' : 'DISABLED', ndaLegalStatus: `${ndaState.approved} APPROVED · ${ndaState.total} TOTAL`, privacyLegalStatus: privacyStatus, emailVerification: emailVerificationProvider, adminMfa: `${adminMfaRequired ? 'REQUIRED' : 'OPTIONAL'} · ${mfa.enrolled}/${mfa.members} ENROLLED`, smtp: smtpConfigured ? 'CONFIGURED_AND_ARCHIVED' : env('SMTP_HOST') ? 'INCOMPLETE' : 'NOT_CONFIGURED', temporaryDevAccess: devLoginEnabled && devLoginExpiry > Date.now() ? 'ENABLED' : 'DISABLED', https: cookieSecure ? 'REQUIRED' : 'LOCAL_HTTP', secureCookies: cookieSecure, trustedProxy: 'CONTROLLED_NGINX_ONLY', activeSessions: activeSessions.rows[0].count, securityEvents7d: securityEvents.rows[0].count, productionReady: externalEnabled && Number(ndaState.approved) > 0 && privacyStatus === 'APPROVED' && emailVerificationProvider !== 'NONE' && cookieSecure && adminMfaRequired && Number(mfa.enrolled) === Number(mfa.members) && smtpConfigured && !devLoginEnabled}; });
  ...
  return {database: 'postgres', ndaStatus: startupNdaStatus, privacyStatus, externalEnabled, workflowTestEnabled, health: async () => ({database: (await pool.query('SELECT 1')).rowCount === 1 ? 'ok' : 'error', portal: 'ok', mail: smtpConfigured ? 'configured' : env('SMTP_HOST') ? 'incomplete' : 'disabled', encryption: 'configured'})};
}
