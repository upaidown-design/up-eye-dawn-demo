...
let exitCode=1;
try{
  if(managed)run(['up','-d','--build','--wait']);
  const child=spawn('pnpm',['--filter','@ued/web','exec','playwright','test','--reporter=line'],{stdio:'inherit',env:{...process.env,PLAYWRIGHT_BASE_URL:process.env.PLAYWRIGHT_BASE_URL??'http://127.0.0.1:8090/demo',E2E_ORIGIN:process.env.E2E_ORIGIN??'http://127.0.0.1:8090',E2E_ADMIN_EMAIL:process.env.E2E_ADMIN_EMAIL??'e2e-owner@example.invalid',E2E_ADMIN_PASSWORD:process.env.E2E_ADMIN_PASSWORD??'E2E-Owner-Only-2026!'}});
